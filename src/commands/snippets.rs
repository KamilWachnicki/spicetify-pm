...
pub async fn run_list(http: &HttpClient, json: bool) -> Result<()> {
...
pub async fn run_installed(http: &HttpClient, json: bool) -> Result<()> {
    let enabled = enabled_keys(&Ledger::load()?);
    if json {
...
/// and the pager's digit selection in `snippets list`.
...
/// uninstalls so the auto-generated file is always rebuilt from scratch.
...
fn find_snippet<'a>(snippets: &'a [Snippet], name: &str) -> Result<&'a Snippet> {
    ...
    snippets
        ...
        .ok_or_else(|| {
            Error::other(format!(
                "no snippet named `{name}`; run `spicepm snippets list`"
            ))
        })
}
...
pub fn generate_js(enabled: &[Snippet]) -> String {
    ...
    format!(
        "// Generated by spicepm {version} — do not edit by hand.\n\
         // Re-run `spicepm snippets add|remove` to change this file.\n\
         (function () {{\n\
         \x20 \"use strict\";\n\
         \x20 function inject() {{\n\
